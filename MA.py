...
def MA(file1):
    try: 
        ...
        ws1.title= "output_"+str(today)+"AWS vs RAMP"
        ws1 = obj.create_sheet("Sheet3")
        ws1.title= "output_"+str(today)+"AWS vs Predix"
        sheet_obj1 = obj["AWS"]
        sheet_obj2 = obj["RAMP"]
        sheet_obj3 = obj["output_"+str(today)+"(AWS vs RAMP)"]
        sheet_obj4 = obj["Predix"]
        sheet_obj5 = obj["output_"+str(today)+"(AWS vs Predix)"]
        
        sheet_obj1.insert_cols(0,amount=1)
        sheet_obj2.insert_cols(0,amount=1)
        sheet_obj4.insert_cols(0,amount=1)
        sheet_obj4.insert_cols(2,amount=1)
        ...
        for i in range(2,n):
            ...
            f10 = '=if(ISNA(VLOOKUP(A'+str(i)+',RAMP!A:L,12,FALSE)),"AWS id not in RAMP",if(len(VLOOKUP(A'+str(i)+',RAMP!A:L,12,FALSE))=0,"",text(DATE(YEAR(VLOOKUP(A'+str(i)+',RAMP!A:L,12,FALSE)),MONTH(VLOOKUP(A'+str(i)+',RAMP!A:L,12,FALSE)),DAY(VLOOKUP(A'+str(i)+',RAMP!A:L,12,FALSE))),"dd/mm/yyyy") ))'#R_cod
            f11 = '=if(ISNA(VLOOKUP(A'+str(i)+',RAMP!A:M,13,FALSE)),"AWS id not in RAMP",if(len(VLOOKUP(A'+str(i)+',RAMP!A:M,13,FALSE))=0,"",text(DATE(YEAR(VLOOKUP(A'+str(i)+',RAMP!A:M,13,FALSE)),MONTH(VLOOKUP(A'+str(i)+',RAMP!A:M,13,FALSE)),DAY(VLOOKUP(A'+str(i)+',RAMP!A:M,13,FALSE))),"dd/mm/yyyy") ))'#cod
