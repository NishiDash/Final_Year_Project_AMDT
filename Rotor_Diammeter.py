...
def RotorDiamter(file1):
    try:
        ...
        sheet_obj['L1']='rotor_diamter(RAMP) wrt id'
        sheet_obj['M1']='rotor_diamter(Predix) wrt id'
        ...
        sheet_obj1['B1']='rotor_diamter(AWS)'
        sheet_obj1['C1']='Turbine_Serial_number(RAMP)'
        sheet_obj1['D1']='rotor_diamter(RAMP)'
        sheet_obj1['E1']='serial number(Predix)'
        sheet_obj1['F1']='rotor_diamter(Predix)'
        ...
        for i in range(2,n):
            ...
            i5 = 'O'+str(i)
            f1 = '=TRIM(B'+str(i)+')'
            f2 = '=IF(ISNA(VLOOKUP(A'+str(i)+',D:F,3,FALSE)),"AWS id not in RAMP",IF(OR(LEN(VLOOKUP(A'+str(i)+',D:F,3,FALSE))=0,VLOOKUP(A'+str(i)+',D:F,3,FALSE)="NULL"),"",ROUND(VLOOKUP(A'+str(i)+',D:F,3,FALSE),6)))'
            f3 = '=IF(AND(ISNA(VLOOKUP(A'+str(i)+',G:K,5,FALSE)),ISNA(VLOOKUP(A'+str(i)+',I:K,3,FALSE))),"AWS id not in Predix",IF(NOT(ISNA(VLOOKUP(A'+str(i)+',G:K,5,FALSE))),IF(OR(LEN(VLOOKUP(A'+str(i)+',G:K,5,FALSE))=0,VLOOKUP(A'+str(i)+',G:K,5,FALSE)="NULL"),"",ROUND(VLOOKUP(A'+str(i)+',G:K,5,FALSE),6)),IF(OR(LEN(VLOOKUP(A'+str(i)+',I:K,3,FALSE))=0,VLOOKUP(A'+str(i)+',I:K,3,FALSE)="NULL"),"",ROUND(VLOOKUP(A'+str(i)+',I:K,3,FALSE),6))))'
