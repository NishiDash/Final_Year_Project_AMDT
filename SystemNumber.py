...
def SystemNumber(file1, attribute):
    try:
        print(Fore.RESET)
        ...
        sheet_obj['L1']='SystemNumber(RAMP) wrt id'
        sheet_obj['M1']='SystemNumber(Predix) wrt id'
        ...
        sheet_obj1['B1']='SystemNumber(AWS)'
        sheet_obj1['C1']='Turbine_Serial_number(RAMP)'
        sheet_obj1['D1']='SystemNumber(RAMP)'
        sheet_obj1['E1']='serial number(Predix)'
        sheet_obj1['F1']='SystemNumber(Predix)'
        ...
        for i in range(2,n):
            ...
            f3 = '=IF(AND(ISNA(VLOOKUP(A'+str(i)+',G:K,5,FALSE)),ISNA(VLOOKUP(A'+str(i)+',I:K,3,FALSE))),"AWS id not in Predix",IF(NOT(ISNA(VLOOKUP(A'+str(i)+',G:K,5,FALSE))),IF(LEN(VLOOKUP(A'+str(i)+',G:K,5,FALSE))=0,"",VLOOKUP(A'+str(i)+',G:K,5,FALSE)),IF(NOT(ISNA(VLOOKUP(A'+str(i)+',I:K,3,FALSE))),IF(LEN(VLOOKUP(A'+str(i)+',I:K,3,FALSE))=0,"",TRIM(VLOOKUP(A'+str(i)+',I:K,3,FALSE))),"")))'
            f4 = '=IF(L'+str(i)+'<>"AWS id not in RAMP",IF(AND(OR(C'+str(i)+'="NULL",C'+str(i)+'=""),OR(L'+str(i)+'="NULL",L'+str(i)+'="")),"matching",IF(TRIM(C'+str(i)+')=L'+str(i)+',"matching","not matching")),"AWS id not in RAMP")'
            f5 = '=IF(M'+str(i)+'<>"AWS id not in Predix",IF(AND(OR(C'+str(i)+'="NULL",C'+str(i)+'=""),OR(M'+str(i)+'="NULL",M'+str(i)+'="")),"matching",IF(TRIM(C'+str(i)+')=M'+str(i)+',"matching","not matching")),"AWS id not in Predix")'
            sheet_obj[i1]=f1
            sheet_obj[i2]=f2
            sheet_obj[i3]=f3
            sheet_obj[i4]=f4
