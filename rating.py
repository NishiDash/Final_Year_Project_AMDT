...
def rating(file1):
    try:
        ...
        for i in range(2,n):
            ...
            f1 = '=TRIM(B'+str(i)+')'
            f2 = '=IF(ISNA(VLOOKUP(A'+str(i)+',D:F,3,FALSE)),"AWS id not in RAMP",IF(OR(LEN(VLOOKUP(A'+str(i)+',D:F,3,FALSE))=0,VLOOKUP(A'+str(i)+',D:F,3,FALSE)="NULL"),"",TRIM(VLOOKUP(A'+str(i)+',D:F,3,FALSE))))'
            f3 = '=IF(ISNA(VLOOKUP(A'+str(i)+',D:G,4,FALSE)),"AWS id not in RAMP",IF(OR(LEN(VLOOKUP(A'+str(i)+',D:G,4,FALSE))=0,VLOOKUP(A'+str(i)+',D:G,4,FALSE)="NULL"),"",TRIM(VLOOKUP(A'+str(i)+',D:G,4,FALSE))))'
            f4 = '=IF(AND(ISNA(VLOOKUP(A'+str(i)+',H:L,5,FALSE)),ISNA(VLOOKUP(A'+str(i)+',J:L,3,FALSE))),"AWS id not in Predix",IF(NOT(ISNA(VLOOKUP(A'+str(i)+',H:L,5,FALSE))),IF(OR(LEN(VLOOKUP(A'+str(i)+',H:L,5,FALSE))=0,VLOOKUP(A'+str(i)+',H:L,5,FALSE)="NULL"),"",TRIM(VLOOKUP(A'+str(i)+',H:L,5,FALSE))),IF(NOT(ISNA(VLOOKUP(A'+str(i)+',J:L,3,FALSE))),IF(OR(LEN(VLOOKUP(A'+str(i)+',J:L,3,FALSE))=0,VLOOKUP(A'+str(i)+',J:L,3,FALSE)="NULL"),"",VLOOKUP(A'+str(i)+',J:L,3,FALSE)),TRIM(VLOOKUP(A'+str(i)+',J:L,3,FALSE)))))'
            f5 = '=IF(M'+str(i)+'="AWS id not in RAMP","AWS id not in RAMP",IF(AND(OR(C'+str(i)+'="",C'+str(i)+'="NULL"),OR(M'+str(i)+'="NULL",M'+str(i)+'="")),"matching",IF(TRIM(C'+str(i)+')=M'+str(i)+',"matching","not matching")))'
            f6 = '=IF(P'+str(i)+'="matching","",IF(N'+str(i)+'="AWS id not in RAMP","AWS id not in RAMP",IF(AND(OR(C'+str(i)+'="NULL",C'+str(i)+'=""),OR(N'+str(i)+'="NULL",N'+str(i)+'="")),"matching",IF(TRIM(C'+str(i)+')=N'+str(i)+',"matching","not matching"))))'
            f7 = '=IF(O'+str(i)+'="AWS id not in Predix","AWS id not in Predix",IF(AND(OR(C'+str(i)+'="",C'+str(i)+'="NULL"),OR(O'+str(i)+'="",O'+str(i)+'="NULL")),"matching",IF(TRIM(C'+str(i)+')=O'+str(i)+',"matching","not matching")))'
