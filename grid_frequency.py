...
def grid_frequency(file1, attribute):
    try:
        ...
        sheet_obj['L1']='grid-frequency(RAMP) wrt id'
        sheet_obj['M1']='grid-frequency(Predix) wrt id'
        ...
        sheet_obj1['B1']='grid-frequency(AWS)'
        sheet_obj1['C1']='Turbine_Serial_number(RAMP)'
        sheet_obj1['D1']='grid-frequency(RAMP)'
        sheet_obj1['E1']='serial number(Predix)'
        sheet_obj1['F1']='grid-frequency(Predix)'
        ...
        for i in range(2,n):
            ...
            f3 = '=IF(AND(ISNA(VLOOKUP(A'+str(i)+',G:K,5,FALSE)),ISNA(VLOOKUP(A'+str(i)+',I:K,3,FALSE))),"AWS id not in Predix",IF(NOT(ISNA(VLOOKUP(A'+str(i)+',G:K,5,FALSE))),IF(OR(LEN(VLOOKUP(A'+str(i)+',G:K,5,FALSE))=0,VLOOKUP(A'+str(i)+',G:K,5,FALSE)="null"),"",ROUND(VLOOKUP(A'+str(i)+',G:K,5,FALSE),6)),IF(NOT(ISNA(VLOOKUP(A'+str(i)+',I:K,3,FALSE))),IF(OR(LEN(VLOOKUP(A'+str(i)+',I:K,3,FALSE))=0,VLOOKUP(A'+str(i)+',I:K,3,FALSE)="null"),"",ROUND(VLOOKUP(A'+str(i)+',I:K,3,FALSE),6)),"Id not in Predix")))'
